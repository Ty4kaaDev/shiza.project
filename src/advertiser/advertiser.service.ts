...
export class AdvertiserService {
    constructor(
        @InjectRepository(AdPost) private readonly adPostRepository: Repository<AdPost>,
        @InjectClickHouse() private readonly clickHouse: ClickHouseClient
    ){}

    async createAdPost(user: User, name: string, url: string, formatHour1: number, formatHour2: number): Promise<AdPost> {
